...
sub commands {
  ...
  return ( { label => 'restore topic', addressed => 1, operator => 0, editable => 0,
             re => qr/^restore topic$/i, callback => \&restore_nonop },
           { label => 'restore topic', addressed => 1, operator => 1, editable => 0,
             re => qr/^restore topic(?: (#\S+))/i, callback => \&restore_op },
         );
}
...
sub restore_topic {
    ...
    unless ( $topics{$chl} ) {
        &say( $chl => "Sorry, $bag->{who}, I don't know what was the earlier topic!" );
        return;
    }
